...
```{r eval=FALSE}
    ...
    } else {source(paste0(path_utils_prepro, "/XCMS.R"))}
```
...
```{r eval=FALSE}
if (grepl("ku_00007", path_rds_xcms_XCMSnExp)) {
     
     source("")
     
} else {source(paste0(path_utils_prepro, "/extract_MS2.R"))}
```
...
## (Optional) CAMERA
<font color = "red">If you would like to get more information related to adduct, please run these lines, but you have to have at least 3 samples</font>
...
params$graphMethod <- "lpc"  #clustering method for resulting correlation graph
...
} else {source(paste0(path_utils_prepro, "/CAMERA.R"))}
